...
from ..DomainServices.RepositoryInterfaces.IScheduledActRepository import (
...
class ReservationService(GenericService, IReservationService):
    ...
    def update(self, id, data):
        ...
        GlobalVariables.update_field("confirmation_change", True)
        return result

    def delete(self, id):
        try:
            entity = self.get_by_id(id)
            result = self.generic_repository.delete(entity)
            GlobalVariables.update_field("confirmation_change", True)
            return result
    ...
    def check_for_consistency(data):
        ...
        total_participantes_actuales = (
            ScheduledActRepository.get_actividades_numparticipantes()
        )
        participantes = 0
    ...
    def get_tasa_confirmacion_por_rangoedad():
        ...
        for rango, (edad_min, edad_max) in rangos_edad.items():
            ...
            reservaciones = Reservacion.objects.filter(
                ...
                confirmadas=Count(
                    Case(When(estado="Confirmado", then=1), output_field=IntegerField())
                ),
                canceladas=Count(
                    Case(When(estado="Cancelado", then=1), output_field=IntegerField())
                ),
            )
        ...
        for rango, datos in resultados.items():
            ...
            print(
                f"  Confirmadas: {datos['confirmadas']} ({datos['proporcion_confirmadas']:.2%})"
            )
            print(
                f"  Canceladas: {datos['canceladas']} ({datos['proporcion_canceladas']:.2%})"
            )
