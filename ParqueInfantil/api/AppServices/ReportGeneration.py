...
class ReportGeneration:
    ...
    def create_pdf(self, file_path):
        ...
        if (
            GlobalVariables.get_instance().qualification_change
            or GlobalVariables.get_instance().confirmation_change
            # or GlobalVariables.get_instance().new_finish_activity
        ):

            if GlobalVariables.get_instance().qualification_change:
                self.graphics.GraficarCalificaciones()
                self.graphics.Graficar_Actividades_Avg_Qualifications()
                GlobalVariables.update_field("qualification_change", False)
            if GlobalVariables.get_instance().confirmation_change:
                self.graphics.GraficarTasaConfirmacionPorRangoEdad()
                GlobalVariables.update_field("confirmation_change", False)
                # if GlobalVariables.get_instance().new_finish_activity:
            self.graphics.Graficar_Actividades_Mas_Participadas()
            self.graphics.Graficar_Uso_De_Recursos()
            # GlobalVariables.update_field("new_finish_activity", False)
            ...
            p.drawString(
                100, 450, "Estadísticas de Reservaciones Confirmadas por Rango de Edad"
            )
            ...
            p.drawString(
                100, 800, "Estadísticas de Reservaciones Canceladas por Rango de Edad"
            )
            ...
            p.drawString(
                100, 450, "Estadísticas de Reservaciones Totales por Rango de Edad"
            )
            ...
            p.drawString(
                100, 450, "Estadísticas de Actividades con mayor participación"
            )
            totales_image_path = os.path.join(
                os.path.dirname(__file__),
                "..",
